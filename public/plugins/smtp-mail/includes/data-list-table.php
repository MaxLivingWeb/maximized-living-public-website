...
class SMTPMail_Data_List_Table extends WP_List_Table {
		...
		switch($column_name){
            ...
            case 'location':
                ...
                if( $params && is_object($params) && isset($params->ip) ) {
                    return '<a href="http://whois.moivui.com/location/'. $params->ip .'" target="_blank" rel="help">'. __('View', 'smtp-mail') .'</a>';
                }
                return '';
            case 'subject':
    ...
    function column_title($item){
        ...
        //Build row actions
        $actions = array(
            // 'edit'      => sprintf('<a href="?page=%s&action=%s&code=%s">Edit</a>',$_REQUEST['page'],'edit',$item['id']),
            'detail'      => sprintf('<a href="?page=%s&tab=%s&action=%s&code=%s">Detail</a>',$_REQUEST['page'],'list','detail',$item['id']),
            'delete'    => sprintf('<a href="?page=%s&tab=%s&action=%s&code=%s">Delete</a>',$_REQUEST['page'],'list','delete',$item['id']),
        );
    ...
    function process_bulk_action() {
		...
		if( $message!='' ){
			echo '<div id="message" class="updated '.$class.' is-dismissible">
    ...
    function prepare_items() {
		...
		$search = (!empty($_REQUEST['s'])) ? $_REQUEST['s'] : ''; //If no sort, default to title
		...
		$query =  'SELECT * FROM '.$table. $where . ' ORDER BY '.$orderby . ' '.$order. ' LIMIT '. (($current_page-1)*$per_page).','.$per_page;
	...
	function display_screen_options()
	{
		...
		foreach( $columns as $name => $title )
		{
			...
			$labels .= '<label><input '.( $checked ?'checked="checked"':'' ).' class="hide-column-tog" name="'.
							$key.'" type="checkbox" id="'.$name.'-hide" value="'.$name.'" >'.$title.'</label>';
		}
	...
	function delete_item()
	{
        ...
        $ids = isset($_GET['customer'])?$_GET['customer']:'';
...
function smtpmail_render_customer_detail_form( $item )
{
    ...
    if( $params && is_object($params) && isset($params->ip) ) {
        $whois_link = 'http://whois.moivui.com/location/' . $params->ip;
    }

    ?>
    <form action="<?php echo '?page='.$_REQUEST['page'];?>&tab=list" method="post" class="smtpmail_detail_form">
        ...
        <p>
            ...
            <input name="subject" type="text" value="<?php echo $subject; ?>" class="inputbox required" />
        </p>
        <p>
        ...
        <p class="buttons">
            ...
            <a class="button button-secondary" href="<?php echo '?page='.$_REQUEST['page'];?>&tab=list"><?php _e('Cancel', 'smtp-mail') ;?></a>
        </p>
    </form>
    <?php
