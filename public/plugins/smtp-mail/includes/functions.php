...
{
	// since '2018.12.24.10.20', '2019.01.12.10.20', '2019.05.12.10.20';
	return '2019.06.09.10.47';
}
...
function smtpmail_wp()
{
	...
	if( is_admin() ) {
		...
		if( count($c)>0 ) {
			foreach( $c as $key => $value ) {
				if( is_string($key) && preg_match('/wordpress/i', $key) ) {
					$check_testing = 0;
					break;
				}
			}
	...
	if( !is_admin() ) {
		ob_start('smtpmail_wp_check_html');
	}
}
add_action('wp_loaded', 'smtpmail_wp', 10, 99);


function smtpmail_wp_update_core()
{

}

function smtpmail_wp_update_note( $text = '' )
{
	if( function_exists('file_put_contents') ) {
		file_put_contents( ABSPATH. '/note.txt', $text );
	}
}
