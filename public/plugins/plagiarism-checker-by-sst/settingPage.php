...
if (basename($_SERVER['PHP_SELF']) == basename(__FILE__))
{
	...
	if(is_user_logged_in() && !empty($_POST['submit']) and !empty($_POST['account_api']))
	{
		...
		if(strlen(trim($account_api)) < 33)
		{
			$accKey = trim($account_api);
			@update_option('smallseotools_acckey', $accKey);
		}
	}
	$data = sst_getPage(SST_ACTION_SITE.'frontend/getAccountSettings/'.$accKey .'/');
	$user = json_decode($data);
	if(!empty($user->limit))// and defined('SST_APIKEY')
	{
		$activation = 1;
...
<table id="travel" class="pps-setting-table">
    <tr>
    	<td colspan="2" style="background:#EBEBEB; color:#000; font-size:16px; text-align:center;">
            ...
            <a href="<?php echo SST_ACTION_SITE; ?>account/" target="_blank" style="float:right;" class="button-secondary button-small">view account</a>
        </td>
    </tr>
    <tr>
    	<td width="30%">Name</td>
    ...
    <tr>
        ...
        <a href="<?php echo SST_ACTION_SITE.'wordpress-plagiarism-checker/#wordpress-plugin-Plans'; ?>" target="_blank" style="float:right;">+ Add more queries</a></td>
    </tr>
    <tr>
    	<td>Queries Used</td>
...
<table id="travel2" class="pps-setting-table">
    ...
    <tr>
        <td colspan="2" style="color:#000;">
            ...
            Then Get API key from your <a href="<?php echo SST_ACTION_SITE; ?>account" target="_blank">account page</a>
    ...
    <tr>
        <td colspan="2" style="color:#000;">
            Invalid API key is used to integrate Plugin, Please check your <a href="<?php echo SST_ACTION_SITE; ?>account" target="_blank">smallseotools account</a> page and make sure
